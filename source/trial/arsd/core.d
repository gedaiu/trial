...
struct LimitedVariant {
	...
	const(char)[] getString() const return {
		...
		switch(contains()) {
			...
			case stringz:
				return trial.arsd.core.stringz(cast(char*) ptr).borrow;
			default:
				Throw(); assert(0);
		}
	}
...
version(HasFile) class AsyncFile : AbstractFile {
	...
	this(NativeFileHandle adoptPreSetup) {
		super(adoptPreSetup);
	}
...
__gshared CleanupQueue cleanupQueue;
...
class Timer {
	...
	version(Windows) {} else
	static void unregister(T)(T urt) {
		if(urt.impl !is null)
			urt.unregister();
	}
...
int indexOf(scope const(char)[] haystack, scope const(char)[] needle) {
...
int indexOf(scope const(ubyte)[] haystack, scope const(char)[] needle) {
...
bool endsWith(scope const(char)[] haystack, scope const(char)[] needle) {
...
bool startsWith(scope const(char)[] haystack, scope const(char)[] needle) {
	if(needle.length > haystack.length)
		return false;
...
enum string dumpParams = q{
	{
		import trial.arsd.core;
		trial.arsd.core.dumpParamsImpl(__FUNCTION__, __traits(parameters));
	}
};
